...
const previewData = {
  ...
  planOffer: 'Excitel Fiber Promo 200 9 plus 3 4620',
...
const createEmptyTemplate = (): CafTemplate => ({
  ...
  website: 'https://justfiber.in',
  termsUrl: 'https://justfiber.in/terms-and-conditions',
  logoDataUrl: '',
  footerLeftLabel: 'ERP Name',
  footerLeftValue: 'JustFiber ERP',
...
const normalizeTemplate = (input: any): CafTemplate => ({
  ...
  brandName: input?.brandName || input?.companyName || 'JustFiber',
  ...
  companyAddress: input?.companyAddress || input?.registeredOffice || '',
  website: input?.website || '',
  termsUrl: input?.termsUrl || input?.website || '',
  logoDataUrl: input?.logoDataUrl || input?.imageUrl || '',
  footerLeftLabel: input?.footerLeftLabel || 'ERP Name',
  footerLeftValue: input?.footerLeftValue || 'JustFiber ERP',
...
export default function CafTemplatesPage() {
  ...
  async function loadTemplates() {
    try {
      ...
      const templateList = Array.isArray(res.data?.value?.cafTemplates)
        ? res.data?.value?.cafTemplates.map(normalizeTemplate)
        : []
  ...
  async function handleDelete() {
    ...
    try {
      ...
      console.error('[caf-templates] Failed to delete template:', error)
          ...
          <div className="card p-6">
            ...
            <div className="grid gap-4">
              ...
              <TextArea
                ...
                onChange={(value) => setForm((prev) => ({ ...prev, declarationText: value }))}
              />
            </div>
          </div>
        ...
        <div className="card p-6">
          <div className="mb-5 flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-sm text-slate-500">Clean CAF preview based on your uploaded sample layout.</div>
            </div>
          ...
          <div className="mx-auto max-w-[920px] rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
            <div className="flex flex-wrap items-start justify-between gap-6 border-b border-slate-200 pb-6">
              <div className="flex items-start gap-4">
                <div
                  className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl text-lg font-semibold text-white"
                ...
                <div>
                  <div className="text-2xl font-semibold text-slate-950">{form.brandName}</div>
                  <div className="mt-1 max-w-md whitespace-pre-line text-sm text-slate-500">{form.companyAddress || 'Organization address'}</div>
                  <div className="mt-2 text-sm text-slate-500">{form.website || 'https://justfiber.in'}</div>
                </div>
              </div>

              <div className="min-w-[220px] rounded-2xl bg-slate-50 px-5 py-4 text-right">
                <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">{form.cafTitle}</div>
                <div className="mt-2 text-lg font-semibold text-slate-950">
                  {form.documentPrefix || 'CAF'}-{previewData.referenceNo.split('-').slice(1).join('-')}
                </div>
            ...
            <div className="mt-6">
              ...
              <PreviewTriple
                first={['Date of birth', previewData.dob]}
                second={['Sex', previewData.gender]}
                third={['Marital status', previewData.maritalStatus]}
              />
              <PreviewTriple
                first={['Mobile', previewData.mobile]}
                second={['Email', previewData.email]}
                third={['Reference', `${form.documentPrefix}-${previewData.referenceNo.split('-').slice(1).join('-')}`]}
              />

              <div className="mt-5">
                <div className="rounded-t-2xl px-4 py-3 text-sm font-semibold text-white" style={{ backgroundColor: form.accentColor }}>
                ...
                <div className="rounded-b-2xl border border-slate-200 border-t-0 px-4">
                  ...
                  <PreviewTriple
                    first={['City', previewData.billingCity]}
                    second={['State', previewData.billingState]}
                    third={['Pin code', previewData.billingPinCode]}
                  />
                </div>
              </div>

              <div className="mt-5">
                <div className="rounded-t-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-900">
                  Permanent address
                </div>
                <div className="rounded-b-2xl border border-slate-200 border-t-0 px-4">
                  <PreviewRow label="Address" value={previewData.permanentAddress} />
                  <PreviewTriple
                    first={['City', previewData.permanentCity]}
                    second={['State', previewData.permanentState]}
                    third={['Pin code', previewData.permanentPinCode]}
                  />
                </div>
              </div>
              ...
              <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="text-sm leading-6 text-slate-700">{form.declarationText}</div>
                <div className="mt-3 break-all text-sm font-medium" style={{ color: form.accentColor }}>
                  {form.termsUrl || 'https://justfiber.in/terms-and-conditions'}
                </div>
              </div>

              <div className="mt-6 grid gap-4 border-t border-dashed border-slate-200 pt-5 md:grid-cols-2">
                <div>
                  <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">{form.footerLeftLabel}</div>
                  <div className="mt-1 text-sm font-semibold text-slate-950">{form.footerLeftValue}</div>
                </div>
                <div className="md:text-right">
                  <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">{form.footerRightLabel}</div>
                  <div className="mt-1 text-sm font-semibold text-slate-950">{form.footerRightValue}</div>
                </div>
              </div>
            </div>
          </div>
